...
function snapPhotoDynamic(rect) {
    ...
    for (let r = 0; r < rows; r++) {
        for (let c = 0; c < cols; c++) {
            ...
            pieces.push({
                img: tempCanvas, 
                x: Math.random() * (canvasElement.width - pW), 
                y: Math.random() * (canvasElement.height - pH),
                targetX: rect.x + (c * pW), 
                targetY: rect.y + (r * pH), 
...
function onResults(results) {
    ...
    if (results.multiHandLandmarks) {
        ...
        for (const landmarks of results.multiHandLandmarks) {
            ...
            landmarks.forEach(pt => { 
                allX.push(pt.x * canvasElement.width); 
                allY.push(pt.y * canvasElement.height); 
            });
        ...
        if (!isCaptured) {
            if (results.multiHandLandmarks.length === 2 && !isCountingDown) {
                lockedRect = { 
                    ...
                    w: Math.max(...allX) - Math.min(...allX), h: Math.max(...allY) - Math.min(...allY) 
                };
                if (pinches === 2) startCountdown(lockedRect);
            }

            if (lockedRect) {
                canvasCtx.strokeStyle = isCountingDown ? '#adff2f' : 'rgba(173, 255, 47, 0.5)';
                canvasCtx.lineWidth = 4;
                canvasCtx.setLineDash(isCountingDown ? [] : [10, 5]);
            ...
            if (isCountingDown) {
                ...
                canvasCtx.fillStyle = "#adff2f"; canvasCtx.font = "bold 120px sans-serif"; canvasCtx.textAlign = "center";
    ...
    pieces.forEach(p => { 
        ...
        if(!p.isLocked) { canvasCtx.strokeStyle = "#adff2f"; canvasCtx.lineWidth = 2; canvasCtx.strokeRect(p.x, p.y, p.w, p.h); }
    });
    canvasCtx.restore();
}
