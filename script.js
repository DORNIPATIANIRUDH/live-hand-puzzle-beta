...
async function initGame() {
    // Check for library readiness (Fixes "Camera is not defined")
    if (typeof Camera === 'undefined') {
        alert("AI libraries are still loading. Please wait 3 seconds.");
        return;
    }
    ...
    try {
        ...
        const camera = new Camera(videoElement, {
            onFrame: async () => {
                // Check video dimensions (Fixes "Aborted" error)
...
function onResults(results) {
    ...
    if (results.multiHandLandmarks) {
        ...
        for (const h of results.multiHandLandmarks) {
            if (getDist(h[4], h[8]) < 0.08) pinches++;
            h.forEach(p => { allX.push(p.x * canvasElement.width); allY.push(p.y * canvasElement.height); });
            drawConnectors(canvasCtx, h, HAND_CONNECTIONS, {color: '#adff2f', lineWidth: 2});
        }
    ...
    pieces.forEach(p => { canvasCtx.drawImage(p.img, p.x, p.y); canvasCtx.strokeStyle = "white"; canvasCtx.strokeRect(p.x, p.y, p.w, p.h); });
    if (lockedRect && !isCaptured) { canvasCtx.strokeStyle = "#adff2f"; canvasCtx.lineWidth = 3; canvasCtx.strokeRect(lockedRect.x, lockedRect.y, lockedRect.w, lockedRect.h); }
    canvasCtx.restore();
}
